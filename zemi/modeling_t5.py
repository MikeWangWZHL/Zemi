# Modified from https://github.com/panx27/finetune-t5/blob/wangz3/training/modeling_t5.py (v4.19.2)
# 
# 
# coding=utf-8
...
from modeling_xattn import GatedCrossAttentionBlock, PerceiverResampler, CrossAttentionBlock
from modeling_xattn import freeze_all_layers_, unfreeze_all_layers_

@add_start_docstrings("""Keep Encoder-Decoder intact, adding small resampler and xattn in-between""", T5_START_DOCSTRING)
...
@add_start_docstrings("""FiD baseline""", T5_START_DOCSTRING)
...
@add_start_docstrings("""T5ForConditionalGenerationMultiAug but without tanh gate in cross attention""", T5_START_DOCSTRING)
class T5ForConditionalGenerationMultiAug_NoTanhGate(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight",
        r"lm_head\.weight",
    ...
    def __init__(self,
        ...
        *,
        ...
        if num_xattn_layers == 1:
            ...
            for i, ins_i in enumerate(inserting_xattn_positions):
                layers.insert(ins_i+i, CrossAttentionBlock(
                    dim=perceiver_xattn_config["dim"], 
                    dim_head=perceiver_xattn_config["dim_head"], 
                    heads=perceiver_xattn_config["heads"],
                    ff_mult=perceiver_xattn_config["xattn_ff_mult"],
    ...
    def encode(self,
        ...
        **kwargs
        ...
        xattn_hidden_states = input_encoder_outputs[0] # encoded input
        for i, layer_module in enumerate(self.xattn_stack):
            if isinstance(layer_module, CrossAttentionBlock):
                xattn_hidden_states = layer_module(
                    xattn_hidden_states,
                    aug_hidden_states,
                    aug_exist_idx = aug_exist_idx
...
@add_start_docstrings("""T5ForConditionalGenerationMultiAug but with additional seperated frozen encoder for augmentations""", T5_START_DOCSTRING)
